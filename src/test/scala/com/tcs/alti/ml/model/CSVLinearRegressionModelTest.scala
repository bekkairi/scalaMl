...
class CSVLinearRegressionModelTest extends FunSuite {
  ...
  test("test five") {
    ...
    val train = getClass.getClassLoader.getResource("model/regression/machine.data")
    val cSVLinearRegressionModel = new CSVLinearRegressionModel("train5", None)


    cSVLinearRegressionModel.loadFromFile(train.getFile)
    sql" delete from ml_model where name='train5'".update().apply()


    MlModelDAO.saveModel(cSVLinearRegressionModel.name, MlType.LINEAR_REGRESSION, cSVLinearRegressionModel)
    assert(cSVLinearRegressionModel.model.gradient().gradient().shape() sameElements Array(1, 10))

    val ret = cSVLinearRegressionModel.predict("gould,concept:32/8780,75,2000,16000,128,1,38,259,157")
